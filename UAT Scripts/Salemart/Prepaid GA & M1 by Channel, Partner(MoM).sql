...
(
	SELECT tm_key_mth, tm_key_day, day, days_in_month--, mom_day
		...
		, product, group_channel, tds_special_channel, partner_code, partner_name
		, ga, m1
	FROM (
...
(
	SELECT tm_key_mth, product, group_channel, tds_special_channel
		-- , group_channel||'_'||tds_special_channel AS channel_name
		, ga_mtd_channel, prev_ga_mtd_channel, ga_mom_channel
		-- --Ranking by GA
		-- , DENSE_RANK() OVER (PARTITION BY tm_key_mth ORDER BY ga_mtd_channel DESC NULLS LAST, group_channel||'_'||tds_special_channel) AS ga_top_channel_rnk
		-- , DENSE_RANK() OVER (PARTITION BY tm_key_mth ORDER BY ga_mtd_channel NULLS LAST, group_channel||'_'||tds_special_channel DESC) AS ga_bot_channel_rnk
		--Ranking by GA %MoM
		, DENSE_RANK() OVER (PARTITION BY tm_key_mth ORDER BY ga_mom_channel NULLS LAST, group_channel||'_'||tds_special_channel DESC) AS ga_top_channel_rnk
		, DENSE_RANK() OVER (PARTITION BY tm_key_mth ORDER BY ga_mom_channel DESC NULLS LAST, group_channel||'_'||tds_special_channel) AS ga_bot_channel_rnk
		, m1_mtd_channel, prev_m1_mtd_channel, m1_mom_channel
		-- --Ranking by M1
		-- , DENSE_RANK() OVER (PARTITION BY tm_key_mth ORDER BY m1_mtd_channel DESC NULLS LAST, group_channel||'_'||tds_special_channel) AS m1_top_channel_rnk
		-- , DENSE_RANK() OVER (PARTITION BY tm_key_mth ORDER BY m1_mtd_channel NULLS LAST, group_channel||'_'||tds_special_channel DESC) AS m1_bot_channel_rnk
		--Ranking by M1 %MoM
		, DENSE_RANK() OVER (PARTITION BY tm_key_mth ORDER BY m1_mom_channel NULLS LAST, group_channel||'_'||tds_special_channel DESC) AS m1_top_channel_rnk
		, DENSE_RANK() OVER (PARTITION BY tm_key_mth ORDER BY m1_mom_channel DESC NULLS LAST, group_channel||'_'||tds_special_channel) AS m1_bot_channel_rnk
	FROM (
		SELECT tm_key_mth, product, group_channel, tds_special_channel
...
(
	...
	FROM (
		SELECT tm_key_mth, product, group_channel, tds_special_channel, partner_code, partner_name
			, ga_mtd
			, LAG(ga_mtd) OVER (PARTITION BY group_channel, tds_special_channel, partner_code ORDER BY tm_key_mth) AS prev_ga_mtd
			, m1_mtd
			, LAG(m1_mtd) OVER (PARTITION BY group_channel, tds_special_channel, partner_code ORDER BY tm_key_mth) AS prev_m1_mtd
		FROM (
			SELECT tm_key_mth, product, group_channel, tds_special_channel, partner_code, partner_name
				, SUM(ga) AS ga_mtd
				, SUM(m1) AS m1_mtd
			FROM W_PREPAID
			WHERE mom_flag = 'Y'
			GROUP BY tm_key_mth, product, group_channel, tds_special_channel, partner_code, partner_name
		) T1
	) T2
	WHERE tm_key_mth = (SELECT curr_tm_key_mth FROM W_PARAM)
) --> W_TXN_MOM
...
-- ORDER BY tm_key_mth, group_channel, tds_special_channel, ga_mtd DESC
-----------------------------------------------------------------------------------------------------------------------
...
-- ORDER BY tm_key_mth, product, group_channel, tds_special_channel, ga_mtd DESC
-----------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------


--> MTD Summary by channel, partner
...
-- ORDER BY tm_key_mth, product, group_channel, tds_special_channel, ga_mtd DESC, partner_name
