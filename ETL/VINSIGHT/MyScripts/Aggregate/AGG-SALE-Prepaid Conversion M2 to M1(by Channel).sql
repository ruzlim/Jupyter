...
FROM (
	...
	SELECT M2.TM_KEY_DAY
		...
		, CASE WHEN COALESCE(M1.ACTUAL_AGG,0) <> 0 THEN (M2.ACTUAL_AGG / M1.ACTUAL_AGG) * 100 END METRIC_VALUE -->> Calculation
	...
	SELECT M2_D.TM_KEY_DAY
		...
		, CASE WHEN COALESCE(M1_D.ACTUAL_AGG,0) <> 0 THEN (M2_D.ACTUAL_AGG / M1_D.ACTUAL_AGG) * 100 END METRIC_VALUE -->> Calculation
	...
	SELECT M2_T.TM_KEY_DAY
		...
		, CASE WHEN COALESCE(M1_T.ACTUAL_AGG,0) <> 0 THEN (M2_T.ACTUAL_AGG / M1_T.ACTUAL_AGG) * 100 END METRIC_VALUE -->> Calculation
