...
FROM
   (
    -->> B7R000102  Prepaid ARPU = ("B1R000100 Prepaid Revenue" / "B1S000500 Prepaid Usage Subs")
    SELECT A.TM_KEY_DAY, 'B7R000102' AS METRIC_CD, 'Prepaid ARPU' AS METRIC_NAME
        , 'ALL' AS COMP_CD, 'A' AS VERSION, AREA_TYPE, AREA_CD, AREA_NAME
		, SUM(CASE 	WHEN METRIC_CD = 'B1R000100' 
					THEN (CASE WHEN P.PERIODFLAG LIKE 'EM%' THEN ACTUAL_AGG_MTH ELSE (ACTUAL_AGG_MTH / P.DAY_NO) * P.DAYS_IN_MONTH END)
					END) NUMERATOR
	...
	INNER JOIN CDSAPPO.DIM_TIME P 
		ON P.TM_KEY_DAY = A.TM_KEY_DAY 
    WHERE METRIC_CD IN ('B1R000100', 'B1S000500')
    AND A.TM_KEY_DAY >= V_SNAP_TM_KEY
    GROUP BY A.TM_KEY_DAY, AREA_TYPE, AREA_CD, AREA_NAME
	) ALL_REVENUE_ARPU
;
...
FROM
   (
    -->> B7R000101  Postpaid ARPU = ("B2R000100 Postpaid Revenue" / "B2S000500 Postpaid Active Subs")
    SELECT A.TM_KEY_DAY, 'B7R000101' AS METRIC_CD, 'Postpaid ARPU' AS METRIC_NAME
        , 'ALL' AS COMP_CD, 'A' AS VERSION, AREA_TYPE, AREA_CD, AREA_NAME
		, SUM(CASE 	WHEN METRIC_CD = 'B2R000100' 
					THEN (CASE WHEN P.PERIODFLAG LIKE 'EM%' THEN ACTUAL_AGG_MTH ELSE (ACTUAL_AGG_MTH / P.DAY_NO) * P.DAYS_IN_MONTH END)
					END) NUMERATOR
...
FROM
   (
    -->> B7R010101  Postpaid ARPU B2C = ("B2R010100 Postpaid Revenue B2C" / "B2S010500 Postpaid Active Subs B2C")
    SELECT A.TM_KEY_DAY, 'B7R010101' AS METRIC_CD, 'Postpaid ARPU B2C' AS METRIC_NAME
        , 'ALL' AS COMP_CD, 'A' AS VERSION, AREA_TYPE, AREA_CD, AREA_NAME
		, SUM(CASE 	WHEN METRIC_CD = 'B2R010100' 
					THEN (CASE WHEN P.PERIODFLAG LIKE 'EM%' THEN ACTUAL_AGG_MTH ELSE (ACTUAL_AGG_MTH / P.DAY_NO) * P.DAYS_IN_MONTH END)
					END) NUMERATOR
...
FROM
   (
    -->> DB7R000102 Prepaid ARPU : DTAC = ("DB1R000100 Prepaid Revenue : DTAC" / "DB1S000500 Prepaid Usage Subs : DTAC")
    SELECT A.TM_KEY_DAY, 'DB7R000102' AS METRIC_CD, 'Prepaid ARPU : DTAC' AS METRIC_NAME
        , 'DTAC' AS COMP_CD, 'A' AS VERSION, AREA_TYPE, AREA_CD, AREA_NAME
		, SUM(CASE 	WHEN METRIC_CD = 'DB1R000100' 
					THEN (CASE WHEN P.PERIODFLAG LIKE 'EM%' THEN ACTUAL_AGG_MTH ELSE (ACTUAL_AGG_MTH / P.DAY_NO) * P.DAYS_IN_MONTH END)
					END) NUMERATOR
...
FROM
   (
    -->> DB7R000101 Postpaid ARPU : DTAC = ("DB2R000100 Postpaid Revenue : DTAC" / "DB2S000500 Postpaid Active Subs : DTAC")
    SELECT A.TM_KEY_DAY, 'DB7R000101' AS METRIC_CD, 'Postpaid ARPU : DTAC' AS METRIC_NAME
        , 'DTAC' AS COMP_CD, 'A' AS VERSION, AREA_TYPE, AREA_CD, AREA_NAME
		, SUM(CASE 	WHEN METRIC_CD = 'DB2R000100' 
					THEN (CASE WHEN P.PERIODFLAG LIKE 'EM%' THEN ACTUAL_AGG_MTH ELSE (ACTUAL_AGG_MTH / P.DAY_NO) * P.DAYS_IN_MONTH END)
					END) NUMERATOR
...
FROM
   (
    -->> DB7R010101 Postpaid ARPU B2C : DTAC = ("DB2R010100 Postpaid Revenue B2C : DTAC" / "DB2S010500 Postpaid Active Subs B2C : DTAC")
    SELECT A.TM_KEY_DAY, 'DB7R010101' AS METRIC_CD, 'Postpaid ARPU B2C : DTAC' AS METRIC_NAME
        , 'DTAC' AS COMP_CD, 'A' AS VERSION, AREA_TYPE, AREA_CD, AREA_NAME
		, SUM(CASE 	WHEN METRIC_CD = 'DB2R010100' 
					THEN (CASE WHEN P.PERIODFLAG LIKE 'EM%' THEN ACTUAL_AGG_MTH ELSE (ACTUAL_AGG_MTH / P.DAY_NO) * P.DAYS_IN_MONTH END)
					END) NUMERATOR
...
FROM
   (
    -->> TB7R000102 Prepaid ARPU : TMH = ("TB1R000100 Prepaid Revenue : TMH" / "TB1S000500 Prepaid Usage Subs : TMH")
    SELECT A.TM_KEY_DAY, 'TB7R000102' AS METRIC_CD, 'Prepaid ARPU : TMH' AS METRIC_NAME
        , 'TRUE' AS COMP_CD, 'A' AS VERSION, AREA_TYPE, AREA_CD, AREA_NAME
		, SUM(CASE 	WHEN METRIC_CD = 'TB1R000100' 
					THEN (CASE WHEN P.PERIODFLAG LIKE 'EM%' THEN ACTUAL_AGG_MTH ELSE (ACTUAL_AGG_MTH / P.DAY_NO) * P.DAYS_IN_MONTH END)
					END) NUMERATOR
...
FROM
   (
    -->> TB7R000101 Postpaid ARPU : TMH = ("TB2R000100 Postpaid Revenue : TMH" / "TB2S000500 Postpaid Active Subs : TMH")
    SELECT A.TM_KEY_DAY, 'TB7R000101' AS METRIC_CD, 'Postpaid ARPU : TMH' AS METRIC_NAME
        , 'TRUE' AS COMP_CD, 'A' AS VERSION, AREA_TYPE, AREA_CD, AREA_NAME
		, SUM(CASE 	WHEN METRIC_CD = 'TB2R000100' 
					THEN (CASE WHEN P.PERIODFLAG LIKE 'EM%' THEN ACTUAL_AGG_MTH ELSE (ACTUAL_AGG_MTH / P.DAY_NO) * P.DAYS_IN_MONTH END)
					END) NUMERATOR
...
FROM
   (
    -->> TB7R010101 Postpaid ARPU B2C : TMH = ("TB2R010100 Postpaid Revenue B2C : TMH" / "TB2S010500 Postpaid Active Subs B2C : TMH")
    SELECT A.TM_KEY_DAY, 'TB7R010101' AS METRIC_CD, 'Postpaid ARPU B2C : TMH' AS METRIC_NAME
        , 'TRUE' AS COMP_CD, 'A' AS VERSION, AREA_TYPE, AREA_CD, AREA_NAME
		, SUM(CASE 	WHEN METRIC_CD = 'TB2R010100' 
					THEN (CASE WHEN P.PERIODFLAG LIKE 'EM%' THEN ACTUAL_AGG_MTH ELSE (ACTUAL_AGG_MTH / P.DAY_NO) * P.DAYS_IN_MONTH END)
					END) NUMERATOR
...
FROM
   (
    -->> TB7R000103 TOL ARPU = ("TB3R000100 TOL Revenue" / "TB3S000500 TOL Active Subs")
    SELECT A.TM_KEY_DAY, 'TB7R000103' AS METRIC_CD, 'TOL ARPU' AS METRIC_NAME
        , 'TRUE' AS COMP_CD, 'A' AS VERSION, AREA_TYPE, AREA_CD, AREA_NAME
		, SUM(CASE 	WHEN METRIC_CD = 'TB3R000100' 
					THEN (CASE WHEN P.PERIODFLAG LIKE 'EM%' THEN ACTUAL_AGG_MTH ELSE (ACTUAL_AGG_MTH / P.DAY_NO) * P.DAYS_IN_MONTH END)
					END) NUMERATOR
...
FROM
   (
    -->> TB7R000104 TVS ARPU = ("TB4R000100 TVS Revenue" / "TB4S000500 TVS Active Subs")
    SELECT A.TM_KEY_DAY, 'TB7R000104' AS METRIC_CD, 'TVS ARPU' AS METRIC_NAME
        , 'TRUE' AS COMP_CD, 'A' AS VERSION, AREA_TYPE, AREA_CD, AREA_NAME
		, SUM(CASE 	WHEN METRIC_CD = 'TB4R000100' 
					THEN (CASE WHEN P.PERIODFLAG LIKE 'EM%' THEN ACTUAL_AGG_MTH ELSE (ACTUAL_AGG_MTH / P.DAY_NO) * P.DAYS_IN_MONTH END)
					END) NUMERATOR
