...
/***
 	Target : Sales Gross Adds 6 KPIs (re-aggregate for GEO only)
...
/** Aggregate Sctips ***/
	...
	) TARGET_PREPAID_GROSS_ADDS_DTAC
	-----------------------------------------------------------------------------------------------------------------------
	...
	FROM ( 
		...
		AND TM_KEY_DAY >= (SELECT V_DAY_START FROM W_PARAM)
		GROUP BY TM_KEY_DAY, COMP_CD, VERSION, AREA_TYPE, AREA_CD
	) TARGET_POSTPAID_GROSS_ADDS_DTAC
	-----------------------------------------------------------------------------------------------------------------------
	...
	FROM (
		...
		AND TM_KEY_DAY >= (SELECT V_DAY_START FROM W_PARAM)
		GROUP BY TM_KEY_DAY, COMP_CD, VERSION, AREA_TYPE, AREA_CD
	) TARGET_POSTPAID_GROSS_ADDS_TMH
	-----------------------------------------------------------------------------------------------------------------------
