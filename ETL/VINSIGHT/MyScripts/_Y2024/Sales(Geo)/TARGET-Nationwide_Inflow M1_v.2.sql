...
/***
 	Target : Sales Inflow M1 6 KPIs (re-aggregate for GEO only)
...
/** Aggregate Sctips ***/
	...
	FROM ( 
		...
		AND TM_KEY_DAY >= (SELECT V_DAY_START FROM W_PARAM)
		GROUP BY TM_KEY_DAY, COMP_CD, VERSION, AREA_TYPE, AREA_CD
	) TARGET_POSTPAID_INFLOW_M1_DTAC
	-----------------------------------------------------------------------------------------------------------------------
	...
	FROM ( 
		...
		AND TM_KEY_DAY >= (SELECT V_DAY_START FROM W_PARAM)
		GROUP BY TM_KEY_DAY, COMP_CD, VERSION, AREA_TYPE, AREA_CD
	) TARGET_TOL_INFLOW_M1_CONNECTED
	-----------------------------------------------------------------------------------------------------------------------
