...
V_SNAP_TM_KEY NUMBER := TO_NUMBER(20241230) ;
	...
	FROM (
		...
		-->> GEO Channel
			...
			LEFT JOIN ( -->> B1R000900CG Prepaid Inflow M1 - GEO Channel
				...
				WHERE METRIC_CD = 'B1R000900CG'
				AND AREA_TYPE NOT IN ('CCAA', 'CCAATT')
				AND TM_KEY_DAY >= TO_CHAR(ADD_MONTHS(TO_DATE(V_SNAP_TM_KEY), -1), 'YYYYMMDD') -->> Prev MTH Inflow M1 of M2
			) M1
				ON M2.AREA_CD = M1.AREA_CD
				AND M2.PREV_TM_KEY_DAY = M1.TM_KEY_DAY
...
INSERT /*+ APPEND */ INTO GEOSPCAPPO.STG_KPI_NEWCO_COMMON_KPI_ACTUAL 
	...
	FROM (
			-->> B0R0001001GEO Total Service Revenue Growth (YTD YoY%)
			SELECT CURR.TM_KEY_DAY
				, 'B0R0001001GEO' AS METRIC_CD, 'Total Service Revenue Growth (YTD YoY%)' AS METRIC_NAME
				, CASE WHEN COALESCE(PREV.ACTUAL_AGG_YR,0) <> 0 THEN (CURR.ACTUAL_AGG_YR - PREV.ACTUAL_AGG_YR) / PREV.ACTUAL_AGG_YR * 100 END METRIC_VALUE -->> Calculation
				, 'ALL' AS COMP_CD, 'A' AS VERSION
				, CURR.AREA_CD, CURR.AREA_NAME, CURR.AREA_TYPE
				, 'Ref : '||CURR.METRIC_CD||' '||CURR.METRIC_NAME AS REMARK
			FROM (
				SELECT TM_KEY_DAY, METRIC_CD, METRIC_NAME, AREA_TYPE, AREA_CD, AREA_NAME, ACTUAL_AGG_YR
					, SUBSTR(TM_KEY_DAY,5,4) AS MMDD_KEY
				FROM GEOSPCAPPO.AGG_PERF_NEWCO 
			...
			LEFT JOIN (
				SELECT TM_KEY_DAY, METRIC_CD, AREA_CD, ACTUAL_AGG_YR
					, SUBSTR(TM_KEY_DAY,5,4) AS MMDD_KEY
				FROM GEOSPCAPPO.AGG_PERF_NEWCO 
				WHERE METRIC_CD = 'B0R000100GEO' --Total Revenue (Geo)
				AND AREA_TYPE NOT IN ('CCAA', 'CCAATT')
				AND TM_KEY_DAY BETWEEN SUBSTR(V_SNAP_TM_KEY,1,4)-1||SUBSTR(V_SNAP_TM_KEY,5,4) AND SUBSTR(V_SNAP_TM_KEY,1,4)-1||'1231' -->> Prev YTD
			) PREV
				ON PREV.METRIC_CD = CURR.METRIC_CD
				AND PREV.AREA_CD = CURR.AREA_CD
				AND PREV.MMDD_KEY = CURR.MMDD_KEY
		) TOTAL_REV_GROWTH
...
INSERT /*+ APPEND */ INTO GEOSPCAPPO.STG_KPI_NEWCO_COMMON_KPI_ACTUAL 
	...
	FROM (
			-->> B0R0001004 Prepaid Revenue Growth (YTD YoY%)
			SELECT CURR.TM_KEY_DAY
				, 'B0R0001004' AS METRIC_CD, 'Prepaid Revenue Growth (YTD YoY%)' AS METRIC_NAME
				, CASE WHEN COALESCE(PREV.ACTUAL_AGG_YR,0) <> 0 THEN (CURR.ACTUAL_AGG_YR - PREV.ACTUAL_AGG_YR) / PREV.ACTUAL_AGG_YR * 100 END METRIC_VALUE -->> Calculation
				, 'ALL' AS COMP_CD, 'A' AS VERSION
				, CURR.AREA_CD, CURR.AREA_NAME, CURR.AREA_TYPE
				, 'Ref : '||CURR.METRIC_CD||' '||CURR.METRIC_NAME AS REMARK
...
INSERT /*+ APPEND */ INTO GEOSPCAPPO.STG_KPI_NEWCO_COMMON_KPI_ACTUAL 
	...
	FROM (
			-->> B0R0101003 Postpaid Revenue Growth B2C (YTD YoY%)
			SELECT CURR.TM_KEY_DAY
				, 'B0R0101003' AS METRIC_CD, 'Postpaid Revenue Growth B2C (YTD YoY%)' AS METRIC_NAME
				, CASE WHEN COALESCE(PREV.ACTUAL_AGG_YR,0) <> 0 THEN (CURR.ACTUAL_AGG_YR - PREV.ACTUAL_AGG_YR) / PREV.ACTUAL_AGG_YR * 100 END METRIC_VALUE -->> Calculation
				, 'ALL' AS COMP_CD, 'A' AS VERSION
				, CURR.AREA_CD, CURR.AREA_NAME, CURR.AREA_TYPE
				, 'Ref : '||CURR.METRIC_CD||' '||CURR.METRIC_NAME AS REMARK
...
INSERT /*+ APPEND */ INTO GEOSPCAPPO.STG_KPI_NEWCO_COMMON_KPI_ACTUAL 
	...
	FROM (
			-->> B0R0001005 TOL Revenue Growth (YTD YoY%)
			SELECT CURR.TM_KEY_DAY
				, 'B0R0001005' AS METRIC_CD, 'TOL Revenue Growth (YTD YoY%)' AS METRIC_NAME
				, CASE WHEN COALESCE(PREV.ACTUAL_AGG_YR,0) <> 0 THEN (CURR.ACTUAL_AGG_YR - PREV.ACTUAL_AGG_YR) / PREV.ACTUAL_AGG_YR * 100 END METRIC_VALUE -->> Calculation
				, 'TRUE' AS COMP_CD, 'A' AS VERSION
				, CURR.AREA_CD, CURR.AREA_NAME, CURR.AREA_TYPE
				, 'Ref : '||CURR.METRIC_CD||' '||CURR.METRIC_NAME AS REMARK
