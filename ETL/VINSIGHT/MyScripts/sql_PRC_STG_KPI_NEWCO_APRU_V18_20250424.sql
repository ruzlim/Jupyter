...
/***
	Aggregate : Monthly Revenue ARPU : 11 KPIs (C, P, G, H, HH)
...
FROM ( 
	-->> B7R000101 Postpaid ARPU = ("B2R000100 Postpaid Revenue" / "B2S000500 Postpaid Active Subs")
	SELECT START_DAY, TM_KEY_DAY, 'B7R000101' AS METRIC_CD, 'Postpaid ARPU' AS METRIC_NAME, 'ALL' AS COMP_CD, 'A' AS VERSION
		, AREA_TYPE, AREA_CD, AREA_NAME
		, '"B2R000100 Postpaid Revenue(30 Day)" / "B2S000500 Postpaid Active Subs"' AS REMARK
	...
	FROM (
		...
		FROM (
			SELECT TM_KEY_DAY, METRIC_CD, METRIC_NAME, AREA_TYPE, AREA_CD, AREA_NAME
				...
				, CASE WHEN METRIC_CD = 'B2R000100' THEN MAX(ACTUAL_AS_OF) OVER(PARTITION BY METRIC_CD) END CURR_REV_DAY
				, CASE WHEN METRIC_CD = 'B2S000500' THEN ACTUAL_AGG_MTH END SUBS
			FROM GEOSPCAPPO.AGG_PERF_NEWCO 
...
FROM ( 
	-->> DB7R000101 Postpaid ARPU : DTAC = ("DB2R000100 Postpaid Revenue : DTAC" / "DB2S000500 Postpaid Active Subs : DTAC")
	SELECT START_DAY, TM_KEY_DAY, 'DB7R000101' AS METRIC_CD, 'Postpaid ARPU : DTAC' AS METRIC_NAME, 'DTAC' AS COMP_CD, 'A' AS VERSION
		, AREA_TYPE, AREA_CD, AREA_NAME
		, '"DB2R000100 Postpaid Revenue : DTAC(30 Day)" / "DB2S000500 Postpaid Active Subs : DTAC"' AS REMARK
		, COALESCE(SUM(REV_MTH), LAG(SUM(REV_MTH)) IGNORE NULLS OVER(PARTITION BY AREA_CD ORDER BY TM_KEY_DAY)) AS NUMERATOR
		, SUM(SUBS) DENOMINATOR
		, MAX(CURR_REV_DAY) CURR_REV_DAY
	FROM (
		...
		FROM (
			SELECT TM_KEY_DAY, METRIC_CD, METRIC_NAME, AREA_TYPE, AREA_CD, AREA_NAME
				, TO_CHAR(ADD_MONTHS(TO_DATE(TM_KEY_DAY,'YYYYMMDD'),-1)+1,'YYYYMMDD') AS START_DAY
				, CASE WHEN METRIC_CD = 'DB2R000100' THEN MAX(ACTUAL_AS_OF) OVER(PARTITION BY METRIC_CD) END CURR_REV_DAY
				, CASE WHEN METRIC_CD = 'DB2S000500' THEN ACTUAL_AGG_MTH END SUBS
			FROM GEOSPCAPPO.AGG_PERF_NEWCO 
		...
		LEFT JOIN GEOSPCAPPO.AGG_PERF_NEWCO B
			ON B.METRIC_CD = 'DB2R000100'
			AND B.METRIC_CD = A.METRIC_CD
			AND B.AREA_CD = A.AREA_CD
			AND B.TM_KEY_DAY BETWEEN A.START_DAY AND A.TM_KEY_DAY
		GROUP BY A.TM_KEY_DAY, START_DAY, A.METRIC_CD, A.METRIC_NAME, A.AREA_TYPE, A.AREA_CD, A.AREA_NAME, CURR_REV_DAY, SUBS
	) PREP_POSTPAID_REV_ARPU_DTAC
	GROUP BY START_DAY, TM_KEY_DAY, AREA_TYPE, AREA_CD, AREA_NAME
) POSTPAID_REV_ARPU_DTAC
WHERE TM_KEY_DAY >= 20250101 -->> Start calculating from the year 2025 
AND TM_KEY_DAY <= CURR_REV_DAY
;
...
FROM ( 
	-->> TB7R000101 Postpaid ARPU : TMH = ("TB2R000100 Postpaid Revenue : TMH" / "TB2S000500 Postpaid Active Subs : TMH")
	SELECT START_DAY, TM_KEY_DAY, 'TB7R000101' AS METRIC_CD, 'Postpaid ARPU : TMH' AS METRIC_NAME, 'TRUE' AS COMP_CD, 'A' AS VERSION
		, AREA_TYPE, AREA_CD, AREA_NAME
		, '"TB2R000100 Postpaid Revenue : TMH(30 Day)" / "TB2S000500 Postpaid Active Subs : TMH"' AS REMARK
		, COALESCE(SUM(REV_MTH), LAG(SUM(REV_MTH)) IGNORE NULLS OVER(PARTITION BY AREA_CD ORDER BY TM_KEY_DAY)) AS NUMERATOR
		, SUM(SUBS) DENOMINATOR
		, MAX(CURR_REV_DAY) CURR_REV_DAY
	FROM (
		...
		FROM (
			SELECT TM_KEY_DAY, METRIC_CD, METRIC_NAME, AREA_TYPE, AREA_CD, AREA_NAME
				, TO_CHAR(ADD_MONTHS(TO_DATE(TM_KEY_DAY,'YYYYMMDD'),-1)+1,'YYYYMMDD') AS START_DAY
				, CASE WHEN METRIC_CD = 'TB2R000100' THEN MAX(ACTUAL_AS_OF) OVER(PARTITION BY METRIC_CD) END CURR_REV_DAY
				, CASE WHEN METRIC_CD = 'TB2S000500' THEN ACTUAL_AGG_MTH END SUBS
			FROM GEOSPCAPPO.AGG_PERF_NEWCO 
		...
		LEFT JOIN GEOSPCAPPO.AGG_PERF_NEWCO B
			ON B.METRIC_CD = 'TB2R000100'
			AND B.METRIC_CD = A.METRIC_CD
			AND B.AREA_CD = A.AREA_CD
			AND B.TM_KEY_DAY BETWEEN A.START_DAY AND A.TM_KEY_DAY
		GROUP BY A.TM_KEY_DAY, START_DAY, A.METRIC_CD, A.METRIC_NAME, A.AREA_TYPE, A.AREA_CD, A.AREA_NAME, CURR_REV_DAY, SUBS
	) PREP_POSTPAID_REV_ARPU_TMH
	GROUP BY START_DAY, TM_KEY_DAY, AREA_TYPE, AREA_CD, AREA_NAME
) POSTPAID_REV_ARPU_TMH
WHERE TM_KEY_DAY >= 20250101 -->> Start calculating from the year 2025 
AND TM_KEY_DAY <= CURR_REV_DAY
;
...
FROM ( 
	...
	SELECT START_DAY, TM_KEY_DAY, 'B7R010101' AS METRIC_CD, 'Postpaid ARPU B2C' AS METRIC_NAME, 'ALL' AS COMP_CD, 'A' AS VERSION
		, AREA_TYPE, AREA_CD, AREA_NAME
		, '"B2R010100 Postpaid Revenue B2C(30 Day)" / "B2S010500 Postpaid Active Subs B2C"' AS REMARK
		, COALESCE(SUM(REV_MTH), LAG(SUM(REV_MTH)) IGNORE NULLS OVER(PARTITION BY AREA_CD ORDER BY TM_KEY_DAY)) AS NUMERATOR
		, SUM(SUBS) DENOMINATOR
		, MAX(CURR_REV_DAY) CURR_REV_DAY
	FROM (
		...
		FROM (
			SELECT TM_KEY_DAY, METRIC_CD, METRIC_NAME, AREA_TYPE, AREA_CD, AREA_NAME
				...
				, CASE WHEN METRIC_CD = 'B2S010500' THEN ACTUAL_AGG_MTH END SUBS
			FROM GEOSPCAPPO.AGG_PERF_NEWCO 
...
FROM ( 
	...
	SELECT START_DAY, TM_KEY_DAY, 'DB7R010101' AS METRIC_CD, 'Postpaid ARPU B2C : DTAC' AS METRIC_NAME, 'DTAC' AS COMP_CD, 'A' AS VERSION
		, AREA_TYPE, AREA_CD, AREA_NAME
		, '"DB2R010100 Postpaid Revenue B2C : DTAC(30 Day)" / "DB2S010500 Postpaid Active Subs B2C : DTAC"' AS REMARK
		, COALESCE(SUM(REV_MTH), LAG(SUM(REV_MTH)) IGNORE NULLS OVER(PARTITION BY AREA_CD ORDER BY TM_KEY_DAY)) AS NUMERATOR
		, SUM(SUBS) DENOMINATOR
		, MAX(CURR_REV_DAY) CURR_REV_DAY
	FROM (
		...
		FROM (
			SELECT TM_KEY_DAY, METRIC_CD, METRIC_NAME, AREA_TYPE, AREA_CD, AREA_NAME
				...
				, CASE WHEN METRIC_CD = 'DB2S010500' THEN ACTUAL_AGG_MTH END SUBS
			FROM GEOSPCAPPO.AGG_PERF_NEWCO 
...
FROM ( 
	...
	SELECT START_DAY, TM_KEY_DAY, 'TB7R010101' AS METRIC_CD, 'Postpaid ARPU B2C : TMH' AS METRIC_NAME, 'TRUE' AS COMP_CD, 'A' AS VERSION
		, AREA_TYPE, AREA_CD, AREA_NAME
		, '"TB2R010100 Postpaid Revenue B2C : TMH(30 Day)" / "TB2S010500 Postpaid Active Subs B2C : TMH"' AS REMARK
		, COALESCE(SUM(REV_MTH), LAG(SUM(REV_MTH)) IGNORE NULLS OVER(PARTITION BY AREA_CD ORDER BY TM_KEY_DAY)) AS NUMERATOR
		, SUM(SUBS) DENOMINATOR
		, MAX(CURR_REV_DAY) CURR_REV_DAY
	FROM (
		...
		FROM (
			SELECT TM_KEY_DAY, METRIC_CD, METRIC_NAME, AREA_TYPE, AREA_CD, AREA_NAME
				...
				, CASE WHEN METRIC_CD = 'TB2S010500' THEN ACTUAL_AGG_MTH END SUBS
			FROM GEOSPCAPPO.AGG_PERF_NEWCO 
...
FROM ( 
	...
	SELECT START_DAY, TM_KEY_DAY, 'TB7R000103' AS METRIC_CD, 'TOL ARPU' AS METRIC_NAME, 'TRUE' AS COMP_CD, 'A' AS VERSION
		, AREA_TYPE, AREA_CD, AREA_NAME
		, '"TB3R000100 TOL Revenue(30 Day)" / "TB3S000500 TOL Active Subs"' AS REMARK
		, COALESCE(SUM(REV_MTH), LAG(SUM(REV_MTH)) IGNORE NULLS OVER(PARTITION BY AREA_CD ORDER BY TM_KEY_DAY)) AS NUMERATOR
		, SUM(SUBS) DENOMINATOR
		, MAX(CURR_REV_DAY) CURR_REV_DAY
	FROM (
		...
		FROM (
			SELECT TM_KEY_DAY, METRIC_CD, METRIC_NAME, AREA_TYPE, AREA_CD, AREA_NAME
				...
				, CASE WHEN METRIC_CD = 'TB3S000500' THEN ACTUAL_AGG_MTH END SUBS
			FROM GEOSPCAPPO.AGG_PERF_NEWCO 
