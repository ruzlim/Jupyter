...
(
	...
	WHERE VER = 'FINAL' 
	AND AREA_TYPE = 'TH'
	AND TM_KEY_MTH BETWEEN (SELECT V_INI_MTH_START FROM W_PARAM) AND (SELECT V_INI_MTH_END FROM W_PARAM)
	GROUP BY TM_KEY_MTH, AREA_TYPE, AREA_CD
) -->> W_BB_MKS_CORP_INITIAL
-----------------------------------------------------------------------------------------------------------------------

, W_BB_MKS_CCAA_INITIAL AS 
(
	SELECT *
	FROM (
	...
	WHERE TOTAL > 0
) -->> W_BB_MKS_CCAA_INITIAL
-----------------------------------------------------------------------------------------------------------------------

, W_BB_MKS_AGG_INITIAL AS 
(
	-->> P : Nationwide
...
) -->> W_BB_MKS_AGG_INITIAL
-----------------------------------------------------------------------------------------------------------------------

, W_RAW_BROADBAND_MKS_MONTHLY_INITIAL AS
(
-->> % Share
...
-->> No. of Subscriber
	...
	SELECT TM_KEY_MTH, 'NT' AS OPERATOR, 'subs' AS UOM, 'VIN00089' AS METRIC_CD, 'Broadband Subs Share (Subs) : NT' AS METRIC_NAME
		...
		, NULL AS SUBS_VALUE
	FROM W_BB_MKS_AGG_INITIAL
) -->> W_RAW_BROADBAND_MKS_MONTHLY_INITIAL
-----------------------------------------------------------------------------------------------------------------------
-----------------------------------------------------------------------------------------------------------------------
...
FROM (
	...
	FROM W_RAW_BROADBAND_MKS_MONTHLY_INITIAL A
