...
, W_BB_MKS_AGG_1 AS 
(
	SELECT TM_KEY_MTH, ZONE_TYPE, ORGID_G, TDS_SGMD, ORGID_R, TDS_RGM_CODE, ORGID_H, HOP_HINT, TDS_PROVINCE, PROVINCE_CD, PROVINCE_ENG, ORGID_HH, D_CLUSTER, CCAA, DISTRICT_EN, CCAATT, SUB_DISTRICT_EN, EAST_FLAG
...
(
	...
	-->> CCAA : 928 DISTRICT_EN
	SELECT TM_KEY_MTH, 8 AS AREA_NO, 'CCAA' AS AREA_TYPE, CCAA AS AREA_CD, DISTRICT_EN AS AREA_NAME
		, SUM(SUBS_3BB) AS SUBS_3BB, SUM(SUBS_AIS) AS SUBS_AIS, SUM(SUBS_CAT) AS SUBS_CAT, SUM(SUBS_TOL) AS SUBS_TOL, SUM(SUBS_TOT) AS SUBS_TOT, SUM(SUBS_TOTAL) AS SUBS_TOTAL
		, CASE WHEN SUM(SUBS_TOTAL) = 0 OR SUM(SUBS_3BB) = 0 THEN 0 ELSE (SUM(SUBS_3BB) / SUM(SUBS_TOTAL)) * 100 END MKS_3BB
		, CASE WHEN SUM(SUBS_TOTAL) = 0 OR SUM(SUBS_AIS) = 0 THEN 0 ELSE (SUM(SUBS_AIS) / SUM(SUBS_TOTAL)) * 100 END MKS_AIS
		, CASE WHEN SUM(SUBS_TOTAL) = 0 OR SUM(SUBS_CAT) = 0 THEN 0 ELSE (SUM(SUBS_CAT) / SUM(SUBS_TOTAL)) * 100 END MKS_CAT
		...
		, CASE WHEN SUM(SUBS_TOTAL) = 0 OR SUM(SUBS_TOT) = 0 THEN 0 ELSE (SUM(SUBS_TOT) / SUM(SUBS_TOTAL)) * 100 END MKS_TOT
	FROM W_BB_MKS_AGG_1
	GROUP BY TM_KEY_MTH, CCAA, DISTRICT_EN
