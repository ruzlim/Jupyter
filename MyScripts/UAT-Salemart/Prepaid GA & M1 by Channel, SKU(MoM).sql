...
, W_TXN_MOM AS 
(
	SELECT tm_key_mth, product, group_channel, tds_special_channel, group_sim
		, ga_mtd, prev_ga_mtd
	...
	FROM (
		SELECT tm_key_mth, product, group_channel, tds_special_channel, group_sim
			, ga_mtd
			, LAG(ga_mtd) OVER (PARTITION BY group_channel, tds_special_channel, group_sim ORDER BY tm_key_mth) AS prev_ga_mtd
			, m1_mtd
			, LAG(m1_mtd) OVER (PARTITION BY group_channel, tds_special_channel, group_sim ORDER BY tm_key_mth) AS prev_m1_mtd
		FROM (
			SELECT tm_key_mth, product, group_channel, tds_special_channel, group_sim
				, SUM(ga) AS ga_mtd
				, SUM(m1) AS m1_mtd
			FROM W_PREPAID
			WHERE mom_flag = 'Y'
			GROUP BY tm_key_mth, product, group_channel, tds_special_channel, group_sim
		) T1
	) T2
) --> W_TXN_MOM
...
, W_TXN_MTD AS 
(
	SELECT A.tm_key_mth, A.product, A.group_channel, A.tds_special_channel, A.group_sim
...
FROM (
	SELECT tm_key_mth, 'ALL' AS product, 'ALL' AS group_channel, 'ALL' AS tds_special_channel, 'ALL' AS group_sim
		-- Gross Adds
		, ga_mtd
...
-- ORDER BY tm_key_mth, product, group_channel, tds_special_channel, m1_mtd DESC, group_sim
ORDER BY tm_key_mth, product, m1_top_channel_rnk, m1_mtd DESC, group_sim